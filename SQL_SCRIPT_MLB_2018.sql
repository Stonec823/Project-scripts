...
-- Seperating 'name' column into first_name and last_name

SELECT 
	left(name, LOCATE(' ', name)) AS first_name,
	substring(name, LOCATE(' ', name)+1, length(name) - (LOCATE(' ', name)-1)) AS last_name
FROM atbats;
...
UPDATE atbats
SET first_name = left(name, LOCATE(' ', name)) AS first_name, 
SET last_name = substring(name, LOCATE(' ', name)+1, length(name) - (LOCATE(' ', name)-1));

SELECT first_name, last_name
FROM atbats;


-- creating a list of just rookie players using a CTE
